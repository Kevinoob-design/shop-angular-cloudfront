...
export class ProductsService extends ApiService {
	...
	getProductById(id: string): Observable<Product | null> {
		...
		return this.http.get<{ product: Product }>(url).pipe(map(resp => resp.product))
	}
	...
	getProductsForCheckout(ids: string[]): Observable<Product[]> {
		...
		return this.getProducts().pipe(map(products => products.filter(product => ids.includes(product.id))))
	}
}
